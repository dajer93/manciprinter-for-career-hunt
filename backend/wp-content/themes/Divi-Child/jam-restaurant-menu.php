<?php
function jam_divi_child_theme_setup() {

  if ( class_exists('ET_Builder_Module')) {

		class ET_Builder_Module_Jam_Menu extends ET_Builder_Module_Type_PostBased {
			function init() {
				$this->name       = esc_html__( 'Jam Menu', 'et_builder' );
				$this->slug       = 'et_pb_jam_restaurant_menu';
				$this->vb_support = 'on';
			...
			function get_fields() {
				$fields = array(
					...
					'show_title' => array(
						...
						'description'     => esc_html__( 'Turn menu titles on or off.', 'et_builder' ),
					),
					'show_desc' => array(
						...
						'default_on_front' => 'on',
						'toggle_slug'     => 'elements',
						'description'     => esc_html__( 'Turn menu description on or off.', 'et_builder' ),
					),
					'show_price' => array(
						...
						'default_on_front' => 'on',
						'toggle_slug'     => 'elements',
						'description'     => esc_html__( 'Turn menu item price on or off.', 'et_builder' ),
					),
					'show_image' => array(
						...
						'default_on_front' => 'on',
						'toggle_slug'     => 'elements',
						'description'     => esc_html__( 'Turn menu images on or off.', 'et_builder' ),
					),
			...
			function render( $attrs, $content = null, $render_slug ) {
				...
				$show_title         = $this->props['show_desc'];
				$show_desc         	= $this->props['show_title'];
				...
				if ( $portfolio->have_posts() ) {
					while( $portfolio->have_posts() ) {
						...
						if ( '' !== $post->post_thumbnail && 'on' === $show_image ) {
						...
						?>
						<div id="post-<?php echo esc_attr( $post->ID ); ?>" class="<?php echo esc_attr( join( $post->post_class_name, ' ' ) ); ?> jam-menu-item">
							...
							<?php if ( 'on' === $show_price ) { ?>
									<div class="jam-menu-price">
							...
							<?php if ( 'on' === $show_desc ) { ?>
								<?php $post_content = et_strip_shortcodes( et_delete_post_first_video( get_the_content() ), true ); ?>
							...
							*/ ?>
							...
							<?php if ( '' !== $post->post_thumbnail && 'on' === $show_image ) { ?>
							<div class="jam-menu-image">
								<a href="<?php echo esc_url( $post->post_thumbnail ); ?>" title="<?php echo esc_attr( get_the_title() ); ?>">
									<?php if ( 'on' === $fullwidth ) { ?>
										<span class="et_portfolio_image">
											<img src="<?php echo wp_get_attachment_thumb_url( get_post_thumbnail_id($post->ID) ); ?>" alt="<?php echo esc_attr( get_the_title() ); ?>" width="1080" height="9999" />
										</span>
									<?php } else { ?>
										<span class="et_portfolio_image">
											<img src="<?php echo wp_get_attachment_thumb_url( get_post_thumbnail_id($post->ID) ); ?>" alt="<?php echo esc_attr( get_the_title() ); ?>" width="400" height="284" />
											<?php echo $overlay; ?>
										</span>
									<?php } ?>
								</a>
		...
		$et_builder_module_jam_restaurant_menu = new ET_Builder_Module_Jam_Menu();
		add_shortcode( 'et_pb_jam_restaurant_menu', array($et_builder_module_jam_restaurant_menu, '_shortcode_callback') );

	}

}
add_action('et_builder_ready', 'jam_divi_child_theme_setup');
